...
Принятые допущения и решения сохраняются рядом с исходными вопросами. Вопросы без ответа для MVP и последующих этапов остаются открытыми. Общие правила собраны в [decisions.md](decisions.md).
...
7. **Business chat scope:** does the connected bot handle all eligible personal chats or only chats explicitly selected in Telegram?
   > *(Принятое решение для MVP: владелец аккаунта определяет доступные чаты в настройках Telegram Business. Telegram применяет этот выбор и присылает боту-коннектору сообщения только из разрешённых чатов. Наш бэкенд не получает и не хранит отдельный список чатов и обрабатывает все сообщения, которые Telegram прислал через это подключение.)*
