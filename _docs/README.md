...
flowchart LR
    ...
    GAP["Gap Roadmap + Analysis<br/>исследование"] -.->|PM проверяет при груминге| ISS
...
| [Gap Roadmap](research/gap-implementation-roadmap.md), [Gap Analysis](research/gap-analysis.md) | Исследовательские находки и их место в очереди Issues; это не принятые требования. | PM проверяет Roadmap для каждого Issue и открывает Analysis только по связанным gaps или новой проблеме. |
...
1. Открыть Issue, связанную User Story и соответствующие требования SPEC.md. PM проверяет Gap Roadmap для этого Issue; если указан gap, читает его в Gap Analysis.
2. Если выбор неясен или документы расходятся, проверить Decisions, затем Open Questions. Исследовательский gap помогает сформулировать вопрос и варианты, но сам не устанавливает поведение.
3. Если обнаружен новый gap, записать его в Gap Analysis и поставить перед нужным Issue или внутрь него в Roadmap. Текущий Issue задерживается только тогда, когда без ответа нельзя согласовать его критерии.
4. При груминге согласовать принятое поведение в SPEC, Decisions или Open Questions и критериях Issue. При реализации и проверке использовать Issue вместе с этими требованиями; Tech Stack читать, когда затронут технический выбор.
