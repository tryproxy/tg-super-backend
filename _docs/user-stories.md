...
```mermaid
flowchart TD
    A["Клиент пишет боту поддержки"] --> B{"Топик клиента существует?"}
    B -->|Нет или удалён| C["В супергруппе появляется<br/>топик клиента"]
    B -->|Да| D["Обращение попадает<br/>в тот же топик"]
    C --> E["Менеджер видит сообщение<br/>или причину, почему его нельзя передать"]
    D --> E
    E --> F["Менеджер отвечает в топике"]
    F --> G{"Ответ доставлен?"}
    G -->|Да| H["Клиент получает ответ<br/>от бота поддержки"]
    G -->|Ошибка или результат неизвестен| I["Менеджер видит предупреждение<br/>в этом топике"]
```
