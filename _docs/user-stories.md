...
Критерии приёмки: владелец выбирает в Telegram переписки для поддержки; только сообщения из них появляются в супергруппе.
Ответ менеджера приходит от Business-аккаунта; если Telegram отклоняет отправку (например, спустя сутки без сообщения клиента), менеджер видит причину в топике.
...
flowchart TD
    ...
    F -->|Business| H{"Telegram принял ответ?"}
    H -->|Да| I["Клиент получает ответ<br/>от Business-аккаунта"]
    H -->|Нет| J["Менеджер видит причину<br/>в топике"]
```
...
### US-12 — Автоматическое приглашение менеджеров

Как владелец продукта, я могу при создании новой супергруппы указать менеджеров из CSV, чтобы система попробовала сразу добавить их в группу с моего Telegram-аккаунта.

Критерии приёмки: Shell показывает, кого удалось добавить, а кого Telegram не позволил добавить или найти. Это не мешает работе группы; для остальных менеджеров остаётся приглашение по ссылке.

### Как владелец создаёт супергруппу в Release

```mermaid
flowchart TD
    A{"У продукта уже есть супергруппа?"} -->|Да| D["Shell показывает существующую группу;<br/>менеджеров приглашают по ссылке"]
    A -->|Нет| B["Shell показывает QR-код"]
    B --> C["Владелец подтверждает вход<br/>в Telegram-приложении"]
    C --> E["Система создаёт закрытую супергруппу<br/>с топиками на аккаунте владельца"]
    ...
    K --> M{"Есть CSV менеджеров?"}
    M -->|Да| N["Система пробует добавить<br/>менеджеров через аккаунт владельца"]
    N --> O["Shell показывает, кто добавлен,<br/>а кому нужно приглашение по ссылке"]
    M -->|Нет| P["Менеджеров можно пригласить позже"]
```
