...
| Release | A separate MTProto component obtains owner authorization through Shell's QR flow and provisions a group under that user's account. It records and reconciles provisioning state without entering the normal webhook message path. Manager onboarding continues through MVP invitation links; direct MTProto invitations are outside scope. Session storage and QR fallback remain open questions. |
