...
For the implementation Issue cycle, the main session selects Issues and delegates PM, Engineer, and QA work using [_docs/workflow/process.md](_docs/workflow/process.md); it does not perform their work. Each role follows its own file in `_docs/workflow/team/`. This delegation rule does not apply to other direct user requests, such as documentation edits.
